            ...
            <div className="modal invite-modal-inner">
                ...
                <div className="modal-body">
                    <div style={{ textAlign: 'center', marginBottom: '20px' }}>
                        <p style={{ fontSize: '13px', color: '#7B8599', lineHeight: '1.5', padding: '0 10px' }}>
                            Download this digital invitation to your device or share it with friends and family via WhatsApp. Present this at the event for fast-track entry.
                        </p>
                    </div>
...
function generateInviteCard(canvas, inviteData, logoImg) {
    ...
    ctx.textAlign = 'right';
    ctx.fillText('INVITATION', W - 70, 74);
    ...
    ctx.letterSpacing = '0px';
    const venueLines = wrapText(ctx, inviteData.venue || 'All Branches', infoX, dtY + 130, pW - 280, 30);

    // Thin separator
    const sep1Y = dtY + 130 + (venueLines * 30) + 10;
    ...
    if (inviteData.countries) {
        ...
        inviteData.countries.split(' • ').forEach(c => {
            ...
            ctx.letterSpacing = '0px';
            const tw = ctx.measureText(c).width + 36;
            if (cx + tw > pX + pW - 50) {
                cx = pX + 210;
                rowY += 50;
            }
            ctx.fillStyle = t.light;
            roundRect(ctx, cx, rowY - 26, tw, 42, 21, true);
            ctx.strokeStyle = t.accent; ctx.lineWidth = 1.2;
            roundRect(ctx, cx, rowY - 26, tw, 42, 21, false); ctx.stroke();
            ctx.fillStyle = t.accent;
            ctx.textAlign = 'center';
            ctx.fillText(c, cx + tw / 2, rowY + 3);
            cx += tw + 12;
        });
        rowY += 64;
    }
    ...
    if (inviteData.activities) {
        ...
        // Split correctly handling newlines, commas, semicolons, and bullet chars
        const acts = inviteData.activities
            .split(/[\n,;•]/)
            .map(act => act.trim())
            .filter(act => act && act.length > 2); // Exclude very short/empty items
        ...
        acts.forEach(act => {
            ...
            ctx.letterSpacing = '0px';
            ctx.textAlign = 'left';
            const lines = wrapText(ctx, act, ax + 30, ay + 3, pW - 280, 30);

            ctx.fillStyle = t.gold;
            ctx.beginPath(); ctx.arc(ax + 10, ay - 4, 9, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#fff';
            ctx.font = 'bold 12px sans-serif';
            ctx.textAlign = 'center';
            ctx.fillText('✓', ax + 10, ay + 1);
